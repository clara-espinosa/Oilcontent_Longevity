...
read.csv("data/oil_regionaldata.csv", sep = ",")%>%
  ...
  labs(tag= "(a)", title= "Species oil content (%)", y= "Oil content (%)")+ #",
...
read.csv("data/oil_regionaldata.csv", sep = ",")%>%
  ...
  labs(tag= "(b)", title = "Oil content vs seed mass relationship", x= "Oil content (%)", y= "Seed mass (mg)")+ 
  theme(text = element_text(family = "sans"),
        plot.title = element_text (size= 12),
        plot.tag = element_text(face="bold"),
...
ggsave(filename = "fig 3. regional patterns.png", plot =fig3 , path = "results/figures", 
       device = "png",dpi = 600, width = 180,units = "mm") # height = 150, scale = 1, 
