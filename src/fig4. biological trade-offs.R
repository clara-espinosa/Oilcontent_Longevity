...
read.csv("data/species_traits.csv")%>% 
  ...
  mutate(order = fct_relevel(order,"Asterales", "Apiales","Lamiales",
                             ...
                             "Saxifragales", "Poales"))%>% 
  ggplot(aes(x=oil.content, y = log(mass_50)))+   
  ...
  labs(tag= "(a)", title= "Seed mass (n=47)", x= "Oil content (%)", y= "Seed mass (log)")+ 
  annotate("text", label="Post. mean: - 0.02\n CI: [ - 0.06 | 0.02 ]", x=29, y=5.5, size=3)+ 
...
read.csv("data/species_traits.csv")%>%
  ...
  mutate(order = fct_relevel(order,"Asterales", "Apiales","Lamiales",
                             ...
                             "Fabales","Malpighiales","Brassicales", "Malvales", 
                             "Saxifragales", "Poales"))%>% 
  ggplot(aes(x=ratio, y = log(mass_50)))+ 
  geom_point(aes(fill =order),shape = 21, size = 4, color = "black")+
  geom_smooth(method= "lm", color= "black", se=F)+
  scale_fill_manual (values=col_order)+
  scale_y_continuous (limits = c(-0.75, 6))+
  ggthemes::theme_tufte(base_size=12) + 
  labs(tag = "", title= "", x= "Ratio UFA/SFA")+ 
...
read.csv("data/species_traits.csv")%>%
  ...
  labs(tag= "(b)",title= "Seed longevity (n=33)", y= "p50 (days)", x = "Oil content (%)")+ 
  ...
  theme (text = element_text(family = "sans"),
         ...
         panel.background = element_rect(color = "black", fill = NULL), 
         legend.position = "none")->fig4B_oil; fig4B_oil 
...
read.csv("data/species_traits.csv")%>%
  ...
  mutate(order = fct_relevel(order,"Asterales", "Apiales","Lamiales", #
                             ...
                             "Fabales","Malpighiales","Brassicales", "Malvales", #
                             "Saxifragales", "Poales"))%>%
  ggplot(aes(y=EHS, x = oil.content))+ 
